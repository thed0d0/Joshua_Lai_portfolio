...
BOOM now the final (maybe) form of the hexapod is complete. The hexapod is not fitted with a pair of mandibles capable of holding onto items and being controlled by the remote controller. In addition I added a LED strip set to red inside the hexapod to give the hexapod more of a evil vibe to strike fear into any opponent it faces. To add the mandibles and LED strip I created custom 3d printed parts, included a seperate 9 volt battery, hooked both mandible serovs and the LED strip to power and the arduino controlling the hexapod, and coded the remote control function for the mandibles. Half of those steps were caused by the emergence of issues along the way. First off, I had two 5 volt pins I was planning to use to power all my modifications on the arduino but, I had 3 electrical components that all required a 5 volt pin. In addition, I learned that the 5 volt pins on the arduino didn't actually provide enough current for the servos causing the servos to spasm out whenever I tried to give it a command. My initial solution was to use a breadboard and a 9 volt battery to power the two servos but another problem arose, I didn't have enough space to fit the breadboard into while keeping the hexapod neat and concise. The solution to that was simple though, instead of using a breadboard, I opted for a proto board with purposfully bridged solders to have it function as a breadboard while being able to be easily cut to a minimal size. That allowed all three modification's electrical parts to be powered and have space in the hexapod. A second challenge I faced was in the making of the mandible mount. Unlike the mandibles themselves the mount for the servos of the mandible took multiple prints to get right. Having to fit a print into a precise fit while having some deviation in size due to 3d printing's innacuracies and some oversights by me that resulted in a massive change in the design caused me to print 4 different versions. To combat this I added extra material into the prints to allow me to manully remove material until it fit perfectly. Lastly, the Code which would have been much harder due to requiring a deep dive to understand the workings of the Freenove developer's code but my peer, Arunabh who also made a hexapod, beat me to it and used his knowledge on the code and Java to massively help me through coding the remote controll of the jaws. Next for the Hexapod Robot of Doom, (maybe) i'll add stronger servos, various sensors, and code to correspond the color of the LEDs to the robot's behaviour. Adding this modification allowed me to learn alot about making custom parts using 3d modeling using onshape, wiring and powering electrical components through the servos and the LED strip, and c++ coding. 
...
# Other Resources/Examples
 ...
 - Arunabh's portfolio (Code helper) : <a href="https://arunabhthakuria01.github.io/Arunabh_BlueStamp_Portfolio/"> Link </a> 
